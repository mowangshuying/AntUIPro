...
AntFrameLessWidget::AntFrameLessWidget(QWidget *parent) : FramelessWidget(parent)
{
    initialize();
    connect(FramelessManager::instance(), &FramelessManager::systemThemeChanged, this, &AntFrameLessWidget::updateStyleSheet);
}

AntFrameLessWidget::~AntFrameLessWidget() = default;

void AntFrameLessWidget::closeEvent(QCloseEvent *event)
{
    FramelessWidget::closeEvent(event);
}

void AntFrameLessWidget::initialize()
{
...
void AntFrameLessWidget::updateStyleSheet()
{
    //  m_contentWidget->setStyleSheet("background-color:pink;");
    update();
}

void AntFrameLessWidget::waitReady()
{
    FramelessWidgetsHelper *helper = FramelessWidgetsHelper::get(this);
    helper->waitForReady();
